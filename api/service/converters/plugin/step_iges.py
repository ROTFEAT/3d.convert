...
class STEPToIGESConverter(BaseConverter):
    """将STEP文件转换为IGES文件的转换器"""
    ...
    def convert(self, input_path: str, output_path: str = None, **kwargs):
        ...
        参数:
            input_path: STEP文件路径
            output_path: 输出IGES文件路径，如果为None则自动生成
            kwargs: 其他可选参数
        """
        # 如果没有提供输出路径，自动生成一个
        if not output_path:
            ...
            # 生成输出文件路径
            output_path = os.path.join(input_dir, f"{input_basename}.iges")
            print(f"[INFO] 未提供输出路径，自动生成: {output_path}")
        
        print(f"[INFO] 开始STEP到IGES转换: {input_path} -> {output_path}")
        ...
        try:
            ...
            if os.path.exists(output_path):
                ...
                print(f"[INFO] IGES文件导出成功: {output_size:.2f} KB")
            else:
                print("[ERROR] 导出失败: 输出文件未创建")
                return False, None
                
            end_time = time.time()
            print(f"[INFO] STEP到IGES转换完成! 用时: {end_time - start_time:.2f} 秒")
            print(f"[SUCCESS] 转换为IGES成功: {output_path}")
            ...
            end_time = time.time()
            print(f"[ERROR] STEP到IGES转换失败: {str(e)}")
...
class IGESToSTEPConverter(BaseConverter):
    """将IGES文件转换为STEP文件的转换器"""
    ...
    def convert(self, input_path: str, output_path: str = None, **kwargs):
        ...
        print(f"[INFO] 开始IGES到STEP转换: {input_path} -> {output_path}")
        start_time = time.time()
        ...
        try:
            ...
            # 导入IGES文件
            print("[INFO] 正在加载IGES文件...")
            Part.insert(input_path, doc.Name)
            print("[INFO] 导入IGES文件成功")
            ...
            end_time = time.time()
            print(f"[INFO] IGES到STEP转换完成! 用时: {end_time - start_time:.2f} 秒")
            ...
            end_time = time.time()
            print(f"[ERROR] IGES到STEP转换失败: {str(e)}")
            print(f"[DEBUG] 异常详情: {traceback.format_exc()}")
            print(f"[INFO] 转换失败! 用时: {end_time - start_time:.2f} 秒")
...
if __name__ == "__main__":
    ...
    if os.path.exists(input_file_path):
        ...
        print(f"[INFO] 测试STEP到IGES转换，使用自动输出路径")
        converter = STEPToIGESConverter()
        success, output_path = converter.convert(test_path)
        print(f"[RESULT] STEP到IGES转换结果: {'成功' if success else '失败'}")
        
        if success:
            print("-" * 40)
            # 测试IGES到STEP转换
            iges_input_path = output_path  # 使用前一步的输出作为输入
            print(f"[INFO] 测试IGES到STEP转换，使用自动输出路径")
            converter = IGESToSTEPConverter()
            success, output_path = converter.convert(iges_input_path)
            print(f"[RESULT] IGES到STEP转换结果: {'成功' if success else '失败'}")
    else:
